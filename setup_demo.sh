...
cleanup() {
    rm -f /tmp/tdx-guest-*.log
    rm -f /tmp/tdx-demo-*-monitor.sock

    PID_VM=$(cat /tmp/tdx-demo-vm-pid.pid)
    PID_TD=$(cat /tmp/tdx-demo-td-pid.pid)

    echo "Cleanup, kill normal vm PID: ${PID_VM}"
    echo "Cleanup, kill TD vm PID: ${PID_TD}"

    kill -TERM ${PID_VM} &> /dev/null
...
echo "TD vm PID: ${PID_TD}. SSH : ssh -p 20022 root@localhost"
echo "Normal vm PID: ${PID_VM}. SSH : ssh -p 10022 root@localhost"
